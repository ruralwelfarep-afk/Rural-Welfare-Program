...
function textWidth(text, size, isBold = false) {
  // Helvetica average char width ratio
...
function drawSectionHeader(page, fonts, y, title) {
  // Increased section header height for bigger font
...
export async function generateApplicationPDF(formData, paymentId, uploadedFiles) {
  ...
  try {
    ...
    if (fs.existsSync(logoPath)) {
      try {
        ...
        const logoImg = await pdfDoc.embedPng(buf)
  ...
  // 8px gap: border starts 8px below yellow stripe bottom
  const YELLOW_BOTTOM = height - headerH - 6
  const GAP_AFTER_YELLOW = 30

  // ── POST + REG NO ─────────────────────────────────────────────────────────────
  // Row starts after gap
  let y = YELLOW_BOTTOM - GAP_AFTER_YELLOW - 4
  const ROW_H   = 22   // increased row height throughout
  const borderStartY = y + ROW_H  // top of border
  ...
  // ── PERSONAL DETAILS — font 10, row 21 ───────────────────────────────────────
  const PERS_FS  = 10    // personal details font size
  const PERS_RH  = 21   // personal row height
  const val1X    = CX + 100
  const col2X    = CX + 258
  const val2X    = CX + 360
  ...
  for (let i = 0; i < personalRows.length; i++) {
    ...
    page.drawText(String(v1 || '—').toUpperCase().substring(0, 22),
                                                  { x: val1X, y, size: PERS_FS, font: regularFont, color: DGRAY })
    page.drawText(l2,                             { x: col2X, y, size: PERS_FS, font: boldFont,    color: GRAY  })
    page.drawText(String(v2 || '—').toUpperCase().substring(0, 18),
                                                  { x: val2X, y, size: PERS_FS, font: regularFont, color: DGRAY })
    y -= PERS_RH
  }
  y -= 6

  // ── EDUCATION TABLE — font 9, row 18 ─────────────────────────────────────────
  ...
  // ── UPLOADED DOCUMENTS — font 10, row 20 ─────────────────────────────────────
  ...
  // ── DATE BOX — date centered properly ──
  ...
  // ── DECLARATION FOOTER — fixed 72px at page bottom ───────────────────────────
  const footerH = 72
  ...
  // ── CONTENT BORDER — drawn LAST (POST row top → box bottom) ──────────────────
  page.drawRectangle({
    x: BX,
    y: borderEndY,
    width: BWIDTH,
    ...
    try {
      ...
      if (fs.existsSync(logoPath)) {
        try {
          ...
          const buf     = await sharp(logoPath).png().toBuffer()
          const logoImg = await pdfDoc.embedPng(buf)
          docPage.drawImage(logoImg, { x: 26, y: 842 - dpH + 9, width: 54, height: 54 })
        } catch {
          const buf     = fs.readFileSync(logoPath)
          const logoImg = await pdfDoc.embedPng(buf)
          docPage.drawImage(logoImg, { x: 26, y: 842 - dpH + 9, width: 54, height: 54 })
